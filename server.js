...
async function createWolvPayInvoice(orderId, amount, currency, description, req) {
  ...
  const resp = await axios.post(`${CRYPTO_CONFIG.wolvpay.apiUrl}/invoice`, payload);
  const inv  = resp.data;
  return {
...
app.post('/register', async (req, res) => {
  ...
  db.run(
    ...
    [username, hash, email],
    err => {
      if (err) return res.render('register', { error: 'User exists' });
      res.redirect('/login');
    }
  );
});
...
app.post('/webhook/wolvpay', express.json(), (req, res) => {
  ...
  if (status.toLowerCase() === 'completed') {
    db.run(
      `UPDATE orders SET payment_status = 'completed', tx_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE payment_id = ?`,
      [txID, invoice_id]
    );
...
// 8. 404 и общ обработчик на грешки
app.use((req, res) => {
